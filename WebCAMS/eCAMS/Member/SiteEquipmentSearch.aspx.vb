...
'<!--$$Author: Bo $-->
'<!--$$Date: 31/12/10 15:51 $-->
'<!--$$Logfile: /eCAMSSource2010/eCAMS/Member/SiteEquipmentSearch.aspx.vb $-->
'<!--$$NoKeywords: $-->
...
'----------------------------------------------------------------
' Namespace: eCAMS
' Class: SiteEquipmentSearch
'
' Description: 
'   Site Equipment search criteria page
'----------------------------------------------------------------

Partial Class SiteEquipmentSearch
...
#End Region
    ...
    Private Sub SetDE()
        ''set page title
        Me.PageTitle = "Site Equipemnt Search"

        ''lblFormTitle.Text = "Search Site Equipment"
        ''Set client Combox
        Call FillCboWithArray(cboClientID, m_DataLayer.CurrentUserInformation.ClientIDs.Split(CType(",", Char)))
        ''Set the selected clientid
        If m_DataLayer.SelectedClientIDForSiteEquipment <> String.Empty Then
            Call ScrollToCboText(cboClientID, m_DataLayer.SelectedClientIDForSiteEquipment)
        End If
    ...
    Private Sub btnSearch_ServerClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.ServerClick
        ...
        m_DataLayer.SelectedClientIDForSiteEquipment = cboClientID.SelectedItem.Value.ToString
        m_DataLayer.SelectedTerminalIDForSiteEquipment = txtTerminalID.Text.ToString.Trim
        m_DataLayer.SelectedPageSize = CType(cboPageSize.SelectedItem.Value, Integer)

        Call Response.Redirect(Request.RawUrl.Replace("SiteEquipmentSearch.aspx", "SiteEquipmentList.aspx"))
    End Sub
