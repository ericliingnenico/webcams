...
if object_id('spWebGetDeviceBySerialNG') is null
	exec ('create procedure dbo.spWebGetDeviceBySerialNG as return 1')
go

Alter Procedure dbo.spWebGetDeviceBySerialNG 
	(
		@UserID int,
		@ClientID varchar(3),
		@Serial varchar(32),
...
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
 ...
 INSERT @MyClientList (ClientID, MMD_ID)
   SELECT ClientID, MMD_ID FROM vwUserClientMMD u JOIN dbo.fnGetSiteClientIDFromDeviceClientID (@ClientID)  s ON u.ClientID = s.SiteClientID
   WHERE UserID = @UserID
 ...
 IF @From = 'EI'
  BEGIN
	 SELECT TOP 1
		...
		Note = @Note + CASE WHEN m.Device LIKE '%SIM%' AND ISNULL(e.PacketNumber, '')<>'' THEN 'SIMCardNo: ' + e.PacketNumber
					ELSE ''
				END,
		TerminalNumber = null,
		CAIC = sm.Acq1CAIC,
		...
		FROM Equipment_Inventory e JOIN @MyClientList t ON e.ClientID = t.ClientID AND e.MMD_ID = ISNULL(t.MMD_ID, e.MMD_ID)
					...
					LEFT OUTER JOIN Site_Maintenance sm ON sm.ClientID IN (SELECT ClientID FROM @MyClientList)
									AND eh.TerminalID = sm.TerminalID
 ...
 IF @From = 'OE'
  BEGIN
	 SELECT TOP 1
		...
		tzDI.TimeZone as DateInTimeZone,
		tzDO.TimeZone as DateOutTimeZone,
		Note = @Note + CASE WHEN m.Device LIKE '%SIM%' AND ISNULL(eh.PacketNumber, '')<>'' THEN 'SIMCardNo: ' + eh.PacketNumber
					ELSE ''
				END,
		TerminalNumber = null,
		CAIC = sm.Acq1CAIC,
 ...
 IF @From = 'SE'
  BEGIN
	 SELECT TOP 1
		...
		dbo.fnFormatDateTimeWD(eh.DateIn) as eqDateIn,
		dbo.fnFormatDateTimeWD(eh.DateOut) As eqDateOut,
		s.MerchantID + ' ' + CASE WHEN s.ClientID <> eh.ClientID THEN s.ClientID ELSE '' END as MerchantID,
		s.[Name],
		tzDI.TimeZone as DateInTimeZone,
		tzDO.TimeZone as DateOutTimeZone,
		Note = @Note + CASE WHEN m.Device LIKE '%SIM%' AND ISNULL(eh.PacketNumber, '')<>'' THEN 'SIMCardNo: ' + eh.PacketNumber
		...
		FROM Site_Equipment e  JOIN @MyClientList t ON e.ClientID = t.ClientID AND e.MMD_ID = ISNULL(t.MMD_ID, e.MMD_ID)
					...
					JOIN M_M_D m ON e.MMD_ID = m.MMD_ID
					LEFT OUTER JOIN Site_Maintenance sm ON sm.ClientID = e.ClientID
									AND e.TerminalID = sm.TerminalID
					LEFT OUTER JOIN Equipment_History eh ON e.Serial = eh.Serial 
...
Grant EXEC on spWebGetDeviceBySerialNG to eCAMS_Role
Go

/*

exec spWebGetDeviceBySerialNG @UserID=210000,@ClientID='CBA',@Serial='9801894',@From='SE'

exec spWebGetDeviceBySerialNG @UserID=210000,@ClientID='CBA',@Serial='14266PP80997312',@From='EI'

exec spWebGetDeviceBySerialNG @UserID=210000,@ClientID='CBA',@Serial='11203PP70565114',@From='OE'
